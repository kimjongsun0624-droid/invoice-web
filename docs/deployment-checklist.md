...
- [ ] `상태` 속성이 **Status 타입**인지 확인 (Select 타입이면 관리자 목록의 상태 필터가 실패함)
- [ ] `조회수`(Number 타입) 속성 추가 — 없어도 앱은 동작하지만 조회수가 항상 0으로 표시됨
...
- [ ] 링크 공유 기능
  - 공유 드롭다운 메뉴 열림
  - 이메일/텔레그램/공유하기(카카오톡 등, 지원 브라우저 한정) 작동
- [ ] 조회수 추적
  - 견적서 페이지 방문 시 관리자 목록의 조회수가 증가
- [ ] 다크모드 전환
  - 테마 버튼 클릭 시 부드럽게 전환
  - 선택한 테마 저장됨
- [ ] PDF 다운로드
  ...
  - 다운로드 성공
- [ ] 모바일 반응형
  - 375px 등 좁은 화면에서 관리자 사이드바가 햄버거 메뉴(Sheet)로 전환되는지 확인
  - 터치 인터랙션 작동
...
> ⚠️ Claude Code의 Playwright MCP는 Chrome 계열 브라우저만 제어할 수 있어 아래 항목은 **자동화 테스트로 검증되지 않았습니다.** 배포 전 수동으로 확인해주세요.

- [ ] Chrome 최신 버전 (Playwright MCP로 기능 확인 완료, 실제 배포본 재확인 권장)
- [ ] Safari 최신 버전 — 수동 확인 필요
- [ ] Firefox 최신 버전 — 수동 확인 필요
- [ ] Edge 최신 버전 — 수동 확인 필요
- [ ] 모바일 Safari (iOS) — 수동 확인 필요
- [ ] 모바일 Chrome (Android) — 수동 확인 필요
...
- 관리자 로그인 시도 제한: IP당 5회/5분
- API 요청(`/api/*`) 제한: IP당 10회/분
- Notion API 호출: Notion 자체 제한(3 requests/second) 준수
